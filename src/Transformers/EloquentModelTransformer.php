...
class EloquentModelTransformer  implements Transformer
{
    ...
    public function __construct(TypeScriptTransformerConfig $config = null)
    {
        if ($config) {
            $this->config = $config;
        }
    ...
    public function getPublicAttributes(ReflectionClass $class): Collection
    {
        ...
        $res = $showModelCommandReflection->getMethod('getAttributes')->invoke(
            ...
            ->map(function ($attr) {
                /*
                // extract the native db field type from "bigint unsigned" or "string(255)"
                preg_match('/^([^ \(]+)/', $attr["type"], $matches);
                $dbFieldType = $matches[1];
                $attr['db_field_type'] = $dbFieldType;
                */
                return $attr;
            });
    ...
    public function transformProperties(
        ...
        return array_reduce(
            ...
            function (string $carry, $property) use ($missingSymbols, $class) {
                //dd($property);
                $transformedType = $this->typeToTypeScript(
                    $this->castToPhpdocumentorType($property['cast'] ?? "encrypted"),
                    $missingSymbols,
                    $class->getName(),
                );
    ...
    public function castToPhpdocumentorType(string $castedAs) : Type
    {
